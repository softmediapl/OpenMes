...
class FiniteSchedulePlanServiceTest extends TestCase
{
    ...
    public function test_it_commits_a_server_calculated_proposal_without_changing_customer_deadline(): void
    {
        ...
        $forecast = WorkOrderForecast::query()->sole();
        $this->assertSame($forecast->id, $workOrder->current_forecast_id);
        $this->assertSame($baseline->id, $forecast->schedule_baseline_id);
        $this->assertSame('2026-08-17 07:00', $forecast->forecast_end_at->format('Y-m-d H:i'));
        $this->assertSame(WorkOrderForecast::RISK_ON_TRACK, $forecast->risk_level);
